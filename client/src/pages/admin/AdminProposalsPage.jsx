...
export default function AdminProposalsPage() {
  ...
  const { data, loading } = useFetch('/api/admin/proposals');
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {proposals.map((p, i) => (
                <tr
                  key={p.id}
                  className={`${i > 0 ? 'border-t border-border' : ''} hover:bg-surface-alt transition-colors cursor-pointer`}
                ...
                >
                  ...
                  <td className="px-5 py-3 text-right">
                    <span className="text-xs text-accent hover:underline">Open →</span>
                  </td>
                </tr>
              ))}
            </tbody>
